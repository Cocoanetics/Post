...
public struct PostConfiguration: Codable, Sendable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    do {
      servers =
        try container.decodeIfPresent([String: ServerConfiguration].self, forKey: .servers) ?? [:]
    } catch {
      if let legacyServers = try? container.decode(
        [LegacyIMAPServerConfiguration].self, forKey: .servers)
      {
  ...
  public static func load(from fileURL: URL = PostConfiguration.defaultFileURL) throws
    -> PostConfiguration
  {
    var explicitServers: [String: ServerConfiguration] = [:]
    var httpPort: Int? = nil

    if FileManager.default.fileExists(atPath: fileURL.path) {
      let data = try Data(contentsOf: fileURL)
      let decoder = JSONDecoder()
      let config = try decoder.decode(PostConfiguration.self, from: data)
      explicitServers = config.servers
      httpPort = config.httpPort
    }

    var mergedServers = explicitServers

    #if canImport(Security)
      let store = KeychainCredentialStore()
      if let keychainItems = try? store.list() {
        for item in keychainItems {
          if mergedServers[item.id] == nil {
            // Auto-generate server entry with default settings
            mergedServers[item.id] = ServerConfiguration()
          }
        }
      }
    #endif

    guard !mergedServers.isEmpty else {
      throw PostConfigurationError.noServersConfigured
    }

    return PostConfiguration(servers: mergedServers, httpPort: httpPort)
  }
...
public enum PostConfigurationError: Error, LocalizedError, Sendable {
  ...
  public var errorDescription: String? {
    ...
    case .noServersConfigured:
      return "No IMAP servers configured in ~/.post.json or the keychain."
