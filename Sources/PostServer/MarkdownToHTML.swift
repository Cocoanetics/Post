...
/// Supports: paragraphs, headings, bold, italic, code, links, blockquotes,
/// unordered/ordered lists, horizontal rules, and line breaks.
enum MarkdownToHTML {

    /// Converts a Markdown string to an HTML document fragment.
    static func convert(_ markdown: String) -> String {
        ...
        while i < lines.count {
            ...
            // Ordered list
            if let _ = orderedListContent(trimmed) {
                var items: [String] = []
                while i < lines.count {
                    let il = lines[i].trimmingCharacters(in: .whitespaces)
    ...
    /// Strips Markdown to plain text (for the text/plain fallback).
    static func stripToPlainText(_ markdown: String) -> String {
        var text = markdown
